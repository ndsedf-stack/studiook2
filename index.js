    ...
    dimanche: {
      ...
      exercises: [
        ...
        { type: 'superset', id: 'superset_dos_pecs', rest: 90, exercises: [
            ...
            { id: 'landminepress', name: 'Landmine Press', sets: 4, reps: '10', rir: 2, startWeight: 35, muscles: { primary: ["Pectoraux", "Épaules"], secondary: ["Triceps"] } }
        ]},
    ...
    for (let week = 0; week < totalWeeks; week++) {
        ...
        ['dimanche', 'mardi', 'vendredi'].forEach(day => {
            ...
            const completedWorkout = {
                ...
                week: week + 1,
                day: day,
                exercises: JSON.parse(JSON.stringify(workoutTemplate.exercises)).map(exo => {
...
const AnatomyChart = ({ history }) => {
        ...
        Object.values(history).forEach(workout => {
            if (new Date(workout.date) >= sevenDaysAgo) {
                workout.exercises.forEach(exo => {
                    ...
                    if (exo.type === 'superset') exo.exercises.forEach(processMuscle);
    ...
    const isMuscleWorked = (muscleName) => {
        return workedMusclesLast7Days.has(muscleName);
    };
